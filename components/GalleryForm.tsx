    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="bg-white border border-slate-200 rounded-3xl p-6 md:p-8 shadow-sm space-y-6">
          ...
          <div>
            <label className="block text-xs font-bold text-slate-400 uppercase mb-2 ml-1">Pilih Sorotan / Album (Opsional)</label>
            <div className="relative">
              <FolderHeart className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
              <select
                value={formData.albumId}
                onChange={(e) => setFormData(prev => ({ ...prev, albumId: e.target.value }))}
                className="w-full pl-10 pr-4 py-3 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-500 transition-all text-slate-900 appearance-none"
              >
                <option value="">Tanpa Album (Hanya Galeri Utama)</option>
                {albums.map((album) => (
            ...
            <p className="mt-2 text-[10px] text-slate-400 italic ml-1">Foto akan dikelompokkan ke dalam sorotan yang dipilih.</p>
          </div>
